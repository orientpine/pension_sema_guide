...
def generate_classification(fund_data_path: str, output_path: str) -> Dict[str, Any]:
    ...
    for fund in funds:
        ...
        category, is_risk_asset, asset_class, region, themes = classify_fund(name, risk_level)

        # Check if hedged
        hedged = '헤지' in name or 'Hedged' in name.lower() or '(H)' in name or '[H]' in name
...
def main():
    ...
    try:
        ...
        print(f"Risk assets: {stats['risk_assets']} ({stats['risk_assets']/stats['total']*100:.1f}%)")
        print(f"Safe assets: {stats['safe_assets']} ({stats['safe_assets']/stats['total']*100:.1f}%)")
        print(f"\nCategory breakdown:")
