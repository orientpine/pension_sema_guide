...
description: "펀드 선택 기준 및 점수 체계. 인덱스 vs 액티브 선택 기준, 핵심-위성 전략, 펀드 선택 점수표, TDF 전용 점수체계(vintage/총보수/설정금액/UH/트랙레코드), 예금 vs 채권 비교, 환헤지 전략을 포함합니다."
---
...
## 4A. TDF 전용 선택 점수체계 (TDF Scorecard)

> **목적**: TDF(Target Date Fund)는 vintage(목표연도) 글라이드패스가 핵심이므로 §4의 일반 점수체계와 **별도**로 평가한다.
> **전제**: 일반 펀드 점수체계(§4), 예금 vs 채권 비교(§6), 인덱스 우선 기준(§1)은 **그대로 유지**되며 본 절은 TDF에만 적용한다.
> **연계**: vintage 매핑·적격성 판정은 `dc-pension-rules` SKILL §2.2·§2.3을 따른다. 70% 한도 산입은 적격TDF면 분자 제외(§2.1).

### 4A.1 점수 항목 및 가중치

| 항목 | 가중치 | 평가 기준 | 비고 |
|------|:------:|----------|------|
| **① vintage 적합도** | 35% | targetYear가 투자자 은퇴 시점에 근접할수록 높은 점수 | **가장 중요** |
| **② 총보수** | 20% | 동일 vintage 내 저비용 우선 (Bogle 원칙) | TDF 상한 ≤ 0.8% (§1.2) |
| **③ 1Y/3Y 수익률** | 15% | 동일 vintage 내 상대 비교 | 결측 시 0% 처리 금지 (§4A.4) |
| **④ 설정금액 (순자산)** | 15% | 소규모 펀드 청산리스크 고려 | 임계값은 파라미터 (§4A.3) |
| **⑤ UH/H 정합성** | 10% | 장기 horizon → UH(환노출) 선호 | §7 환헤지 가이드 연계 |
| **⑥ 위험등급-투자성향 일치** | 5% | 성향에 맞는 위험등급 | 일치=만점 |

> **트랙레코드(⑦)**는 가중 점수에 합산하지 않고 **정보성 플래그**로 별도 표기한다(§4A.4). 3Y 데이터 유무를 사실대로 명시하되, 결측을 점수 페널티로 환산하지 않는다.

### 4A.2 ① vintage 적합도 산출 (가장 중요)

```
투자자 목표연도 우선순위 (dc-pension-rules §2.3 준수):
1. 은퇴예정연도가 입력됨        → 그 연도 직접 사용
2. birthYear + 은퇴연령(입력)   → targetYear = birthYear + 은퇴연령
3. 둘 다 없음 → 폴백            → targetYear = birthYear + 60  (은퇴연령 60세 기본)

vintage 적합도 점수:
  Δ = |fund.targetYear - 투자자_targetYear|   (연 단위)
  Δ = 0년       → 35점 (정확 일치)
  Δ ≤ 5년       → 28점
  Δ ≤ 10년      → 18점
  Δ > 10년      → 8점
...
> 📌 vintage가 투자자 시점보다 **이르면(보수적)** 글라이드패스가 더 안전, **늦으면(공격적)** 더 위험. Δ 동률이면 투자성향에 맞는 방향(공격형=늦은 vintage 허용, 안정형=이른 vintage 선호)을 가산점으로 고려할 수 있다.

### 4A.3 ④ 설정금액 임계값 (파라미터)

> ⚠️ 청산리스크 임계값은 **운영자 조정 가능한 파라미터**로 정의한다. 근거 없는 하드코딩 금지 — 아래 기본값은 시작점이며 시장 상황·데이터에 따라 조정한다.

| 파라미터 | 기본값 | 의미 | 조정 주체 |
|----------|:------:|------|----------|
| `TDF_AUM_MIN` | 100억원 | 미만 시 청산리스크 경고 (감점) | 운영자 |
| `TDF_AUM_SAFE` | 500억원 | 이상 시 안정 규모 (만점) | 운영자 |

```
설정금액 점수 (TDF_AUM_MIN/SAFE 파라미터 기반):
  AUM ≥ TDF_AUM_SAFE    → 15점
  AUM ≥ TDF_AUM_MIN     → 9점
  AUM < TDF_AUM_MIN     → 3점 + 청산리스크 플래그
```

### 4A.4 결측 데이터 처리 (FD4 준수) ⚠️ CRITICAL

> **FD4 원칙**: 결측 장기수익률(3Y 등)을 **0%로 취급하거나 자동 후순위화하지 않는다**.

| 상황 | 금지 | 올바른 처리 |
|------|------|------------|
| 3Y 수익률 결측 | ❌ 0%로 간주하여 감점 | 트랙레코드 플래그 "3Y 미보유"로 **정보 표기**, 수익률 항목은 가용 기간(1Y 등)으로만 비교 |
| 신규 vintage TDF | ❌ 데이터 부족을 자동 후순위 | vintage 적합도가 최우선이므로 적합하면 정상 후보 |
| 1Y/3Y 모두 결측 | ❌ 수익률 0점 페널티 | 수익률 항목(③)을 점수 합산에서 **제외하고 나머지 항목 가중 재정규화** |

```
결측 시 점수 처리 (페널티 금지):
IF 3Y_return is missing:
    트랙레코드 = "3Y 미보유 (정보)"        # 플래그만, 감점 X
    수익률(③)은 1Y 등 가용 기간으로 비교
IF 1Y AND 3Y both missing:
    EXCLUDE 수익률 항목(③) from total
    RENORMALIZE 나머지 항목 가중치 합 = 100%   # 0% 페널티 대신 재정규화
```

### 4A.5 동일 vintage 내 비교 기준

> 같은 targetYear(또는 ±5년 동급) TDF가 복수일 때 우선순위:

```
1순위: 저비용 (총보수 낮은 펀드)            # Bogle 원칙
2순위: 안정 규모 (설정금액 큰 펀드)         # 청산리스크 낮음
3순위: UH/H 정합성 (장기=UH 선호)
4순위: 수익률 (가용 기간 한정, 결측 페널티 X)
선택(권장): 운용사 다변화 — 동률 시 포트폴리오 내 미보유 운용사 가산
```

### 4A.6 TDF 점수 산출 예시 (형식 설명용)

> ⚠️ 아래는 형식 설명용. 실제 TDF는 fund_data.json에서 동적 검색·정렬. 투자자 targetYear = 2050 가정.

| TDF | vintage(35) | 총보수(20) | 수익률(15) | 설정금액(15) | UH/H(10) | 등급(5) | 트랙레코드 | **총점** |
|-----|:-----------:|:----------:|:----------:|:------------:|:--------:|:-------:|:----------:|:------:|
| [TDF2050 A] | 35 | 18 | 12 | 15 | 10 | 5 | 3Y 보유 | **95** |
| [TDF2045 B] | 28 | 16 | 10 | 9 | 6 | 5 | 3Y 보유 | **74** |
| [TDF2050 C (신규)] | 35 | 15 | — (재정규화) | 3 | 10 | 5 | **3Y 미보유 (정보)** | 재정규화 후 비교 |

> TDF2050 C는 3Y 결측이지만 **0% 페널티 없이** 수익률 항목을 제외·재정규화하여 평가한다(FD4).
...
changes:
  ...
  - "v2.1: 섹션 4A 'TDF 전용 선택 점수체계' 추가 (vintage/총보수/수익률/설정금액/UH·H/위험등급)"
  - "v2.1: TDF vintage 적합도를 최우선(35%)으로 정의, dc-pension-rules §2.3 매핑 연계"
  - "v2.1: 설정금액 청산리스크 임계값을 운영자 조정 파라미터(TDF_AUM_MIN/SAFE)로 정의"
  - "v2.1: FD4 준수 — 결측 장기수익률 0% 페널티/자동 후순위화 금지, 재정규화 처리"
  - "v2.1: 트랙레코드를 점수 합산이 아닌 정보성 플래그로 분리"
consumers:
  - fund-portfolio
  - macro-synthesizer
