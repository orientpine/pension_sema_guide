...
|--------|:---------:|:---------------:|:--------:|:---------:|:--------------:|
| [펀드명] | 1 | false | 기타 | Rule 1 | → riskAsset=**true** |

**재판정 근거**: [riskLevel 기준 우선 원칙 설명]

## 안전자산 다각화 검토 (v5.0 신규 - SHOULD, SAFE_ASSET_DECISION="예금" 시)

### 안전자산 다각화 검토 (옵셔널)

| # | 상품 | 유형 | 수익률 | 총보수 | 실질수익 | riskLevel | 원금보장 |
|:-:|------|:----:|------:|------:|--------:|:---------:|:-------:|
| 1 | 과기공 예금 1년 | 예금 | X.X% | 0% | X.X% | - | O |
| 2 | [안전자산 펀드] | 채권형 | X.X% | X.X% | X.X% | 5 | X |

**권고**: [예금 100% 유지 / 혼합 검토 가능]

## 카테고리별 전수 비교 증적 (v5.0 신규 - MANDATORY)
...
(포트폴리오에 편입하는 **모든 카테고리**에 대해 반복)

## UH/H 환헤지 비용 비교 (v5.0 신규 - MANDATORY, UH/H 쌍 존재 시)
...
(동일 펀드의 UH/H 쌍이 모두 존재하는 **모든 경우**에 대해 반복)
...
consumers:
  - fund-portfolio
changes:
  - "v2.0: 데이터 정합성 검증 결과 테이블 템플릿 추가 (v5.0 에이전트 대응)"
